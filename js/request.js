(function () {
        ...
        .forEach((vale) => {
          ...
          const row = `
                    <tr>
                        ...
                        <td><div class="mx-auto text-start" style="width: 8rem">${processStatus(
                          vale.NombreEstado
                        )}</div></td> 
                        <td class="text-center">
                            <button onclick="handleCoordinateButton('${
                              vale.IdVale
                            }')" type="button" class="btn btn-outline-primary">Coordinar</button>
                        </td>
                    </tr>
                `;
  ...
  function processStatus(status) {
    switch (status) {
      case "Pendiente":
        return (
          '<span style="color: orange;font-size: 1.5rem;">■ </span>' + status
        );
        break;
      case "Rechazado":
        return (
          '<span style="color: red; font-size: 1.5rem;">■ </span>' + status
        );
        break;
      case "Aprobado":
        return (
          '<span style="color: blue;font-size: 1.5rem;">■ </span>' + status
        );
        break;
      case "Completado":
        return (
          '<span style="color: green;font-size: 1.5rem;">■ </span>' + status
        );
        break;
      default:
        return status;
