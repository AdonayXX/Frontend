...
document
  .getElementById("togglePassword3")
  .addEventListener("click", function () {
  ...
  .addEventListener("click", function (event) {
    ...
    for (var i = 0; i < sqlKeywords.length; i++) {
      if (tokenUpper.includes(sqlKeywords[i])) {
        showToast(
          "Atención",
          "El token contiene caracteres inválidos, por favor ingrese un valor válido."
        );
        setTimeout(() => {
    ...
    if (contra1 !== contra2) {
      ...
      axios
        .post(
          "http://localhost:18026/api/reset-password/reset-password",
          payload
        )
        .then(function () {
