...
async function loginUser(identificador, Contrasena) {
  try {
    const response = await axios.post(
      `http://localhost:18026/api/usuario/login`,
      {
...
document
  .querySelector("#formUserRegister")
  .addEventListener("submit", function (event) {
    event.preventDefault();
...
function getUserData() {
  const userIdentification = document
    .querySelector("#userIdentification")
    .value.trim();
  const userName = document.querySelector("#userName").value.trim();
  const userFirstLastName = document
    .querySelector("#userFirstlastname")
    .value.trim();
  const userSecondLastName = document
    .querySelector("#userSecondlastname")
    .value.trim();
  const userPassword = document.querySelector("#userPassword1").value.trim();
  const userRegisterEmail = document
    .querySelector("#userRegisterEmail")
    .value.trim();
...
async function consultarCedulaOnBlur() {
  const userIdentification = document
    .querySelector("#userIdentification")
    .value.trim(); // Trim para eliminar espacios en blanco al inicio y final
