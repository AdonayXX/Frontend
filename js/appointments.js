(function () {
    ...
    try {
      ...
      if (pacienteEncontrado) {
        ...
        document.getElementById("primerApellido").value =
          pacienteEncontrado.Apellido1 || "";
        document.getElementById("segundoApellido").value =
          pacienteEncontrado.Apellido2 || "";
        document.getElementById("telefonos").value =
          pacienteEncontrado.Telefono2
            ? `${pacienteEncontrado.Telefono1 || ""} / ${
                pacienteEncontrado.Telefono2
              }`
            : pacienteEncontrado.Telefono1 || "";
        document.getElementById("provincia").value =
          pacienteEncontrado.Provincia || "";
        document.getElementById("canton").value =
          pacienteEncontrado.Canton || "";
        document.getElementById("distrito").value =
          pacienteEncontrado.Distrito || "";
        document.getElementById("barrio").value =
          pacienteEncontrado.Barrio || "";
        document.getElementById("direccion").value =
          pacienteEncontrado.OtrasSenas || "";
  ...
  async function getAcompanantes(identificacion) {
    try {
      ...
      acompanante1Select.innerHTML =
        '<option value="">Seleccionar Acompañante</option>';
      acompanante2Select.innerHTML =
        '<option value="">Seleccionar Acompañante</option>';
      ...
      if (acompanantes && acompanantes.length > 0) {
        ...
        setTimeout(() => {
          showToast(
            "Acompañantes",
            "No se encontraron acompañantes para este paciente."
          );
        }, 1000);
      }
    } catch (error) {
  ...
  function showAcompananteDetails(
    event,
    nombreField,
    apellido1Field,
    telefonoField,
    ...
    const acompanante = acompanantes.find(
      (acompanante) => acompanante.Nombre === selectedOption
    );
    if (acompanante) {
      nombreField.value = acompanante.Nombre || "";
      apellido1Field.value = acompanante.Apellido1 || "";
      telefonoField.value = acompanante.Telefono2
        ? `${acompanante.Telefono1} / ${acompanante.Telefono2}`
        : acompanante.Telefono1 || "";
      parentescoField.value = acompanante.Parentesco || "";
      parentescoField.value = acompanante.Parentesco || "";
    } else {
      showToast(
        "Acompañantes",
        "No se encontró información para el acompañante seleccionado."
      );
    }
  }
  ...
  function handleAcompanante1Change(event) {
    ...
    if (acompanantes && acompanantes.length === 2) {
      acompanante2Select.disabled =
        acompanantes[0].Nombre !== selectedValue1 &&
        acompanantes[1].Nombre !== selectedValue1;
    }

    showAcompananteDetails(
      event,
      document.getElementById("acompananteNombre1"),
      document.getElementById("acompananteApellido1_1"),
      document.getElementById("acompananteTelefono1_1"),
      document.getElementById("acompananteParentesco1")
    );
  }
  ...
  function handleAcompanante2Change(event) {
    ...
    if (acompanantes.length === 2) {
      acompanante1Select.disabled =
        acompanantes[0].Nombre !== selectedValue2 &&
        acompanantes[1].Nombre !== selectedValue2;
    }

    showAcompananteDetails(
      event,
      document.getElementById("acompananteNombre2"),
      document.getElementById("acompananteApellido1_2"),
      document.getElementById("acompananteTelefono1_2"),
      document.getElementById("acompananteParentesco2")
    );
  }

  document
    .getElementById("acompananteNombre1")
    .addEventListener("change", handleAcompanante1Change);
  document
    .getElementById("acompananteNombre2")
    .addEventListener("change", handleAcompanante2Change);
  ...
  document
    ...
    .addEventListener("click", async function (event) {
      ...
      const guardarCita = async () => {
        ...
        const acompanante1Nombre =
          document.getElementById("acompananteNombre1").value;
        const acompanante2Nombre =
          document.getElementById("acompananteNombre2").value;

        const idAcompanante1 = acompanante1Nombre
          ? acompanantes.find(
              (acompanante) => acompanante.Nombre === acompanante1Nombre
            )?.IdAcompanante
          : null;
        const idAcompanante2 = acompanante2Nombre
          ? acompanantes.find(
              (acompanante) => acompanante.Nombre === acompanante2Nombre
            )?.IdAcompanante
          : null;
  ...
  document
    ...
    .addEventListener("change", applyMask);
  document
    .getElementById("identificacion")
    .addEventListener("input", applyMask);
  applyMask();
})();
