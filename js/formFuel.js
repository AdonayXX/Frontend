...
(async function () {
  ...
  const apiUrl = "http://localhost:18026/api/";
  ...
  function clearForm() {
    const formFields = form.querySelectorAll(
      "input:not(#unidades):not(#choferes)"
    );
  ...
  async function checkDuplicateBill(numeroFactura, idRegistro = null) {
    try {
      ...
      return registros.some(
        (registro) =>
          registro.numeroFactura === numeroFactura && registro.id !== idRegistro
      );
    } catch (error) {
  ...
  async function checkDuplicateAutorization(
    numeroAutorizacion,
    idRegistro = null
  ) {
    try {
      ...
      return registros.some(
        (registro) =>
          registro.numeroAutorizacion === numeroAutorizacion &&
  ...
  async function getCurrentMileage(unidad) {
    try {
      ...
      showToast(
        "Error",
        "Error al obtener el kilometraje actual de la unidad."
      );
    }
  }
  ...
  async function updateCurrentMileage(unidad, kilometraje) {
    try {
      ...
      showToast(
        "Error",
        "Error al actualizar el kilometraje actual de la unidad."
      );
    }
  }
  ...
  async function getIdRegistroCombustible(unidad) {
    try {
      const response = await axios.get(
        `${apiUrl}registrocombustible/${unidad}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      const registros = response.data.registro.filter(
        (log) => log.estado === "activo"
      );
  ...
  async function getFuelLog() {
    ...
    try {
      ...
      if (registros.length === 0 && !noSelected) {
        showToast(
          "Error",
          `No hay registros de combustible de la unidad "${unidad}".`
        );
        return;
      } else if (noSelected) {
        return;
      ...
      for (let i = 0; i < tipoCombustibleSelect.options.length; i++) {
        if (
          tipoCombustibleSelect.options[i].text === latestLog.tipoCombustible
        ) {
      ...
      document.getElementById("fecha").value = new Date(latestLog.fecha)
        .toISOString()
        .split("T")[0];
  ...
  async function postFuelLog() {
    ...
    if (await checkDuplicateBill(fuelLogData.numeroFactura, idRegistro)) {
      showToast(
        "Error",
        `Ya existe un registro de combustible con el número de factura ${fuelLogData.numeroFactura}.`
      );
      return;
    } else if (
      await checkDuplicateAutorization(
        fuelLogData.numeroAutorizacion,
        idRegistro
      )
    ) {
      showToast(
        "Error",
        `Ya existe un registro de combustible con el número de autorización ${fuelLogData.numeroAutorizacion}.`
      );
      return;
    }
    ...
    if (await validateFormData(fuelLogData)) {
      ...
      showToast(
        "Éxito",
        `El registro de combustible de la unidad "${fuelLogData.numeroUnidad}" se ha realizado exitosamente.`
      );
    }
  }
  ...
  async function putFuelLog() {
    ...
    if (await validateFormData(fuelLogData)) {
      await axios.put(
        `${apiUrl}registroCombustible/${idRegistro}`,
        fuelLogData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      showToast(
        "Éxito",
        `El registro de combustible de la unidad "${fuelLogData.numeroUnidad}" se ha actualizado exitosamente.`
      );
    }
  }
  ...
  async function deleteFuelLog() {
    ...
    const deleteModal = new bootstrap.Modal(
      document.getElementById("deleteFuelLogModal")
    );
    deleteModal.show();

    document.getElementById("confirmDelete").onclick = function () {
      axios
        .put(`${apiUrl}registroCombustible/${idRegistro}`, fuelLogData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
        .then(() => {
          ...
          showToast(
            "Éxito",
            `El registro de combustible de la unidad se ha eliminado exitosamente.`
          );
        })
        .catch(() => {
  ...
  function getFormData() {
    ...
    return {
      ...
      tipoCombustible:
        tipoCombustibleSelect.options[tipoCombustibleSelect.selectedIndex].text,
  ...
  async function validateFormData(fuelLogData) {
    ...
    if (fuelLogData.kilometraje < kilometrajeActual) {
      showToast(
        "Error",
        `El kilometraje de recarga no puede ser inferior al kilometraje actual de la unidad (${kilometrajeActual} km).`
      );
      return false;
    }
    ...
    if (fuelLogData.fecha > new Date().toISOString().split("T")[0]) {
      showToast(
        "Error",
        "La fecha del registro de combustible no puede ser posterior a la fecha de hoy."
      );
      return;
    }
    ...
    if (
      fuelLogData.hora > horaActual &&
      fuelLogData.fecha === new Date().toISOString().split("T")[0]
    ) {
      showToast(
        "Error",
        "La hora del registro de combustible no puede ser posterior a la hora actual."
      );
      return;
    }
