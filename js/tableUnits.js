...
async function getNombreChofer() {
  try {
    ...
    choferes.forEach((chofer) => {
      choferMap[
        chofer.idChofer
      ] = `${chofer.nombre} ${chofer.apellido1} ${chofer.apellido2}`;
    });
...
async function getTiposRecurso() {
  try {
    const token = localStorage.getItem("token");
    const response = await axios.get("http://localhost:18026/api/tipoRecurso", {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
async function getEstadosUnidad() {
  try {
    ...
    const response = await axios.get(
      "http://localhost:18026/api/estadoUnidad",
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      unidades.forEach((unidad) => {
        if (unidad.idEstado !== 5) {
          ...
          row.innerHTML = `
                        ...
                        <td class="text-center">${(
                          recursoMap[unidad.idTipoRecurso] || "N/A"
                        ).toUpperCase()}</td>
                        <td class="text-center">${
                          choferMap[unidad.choferDesignado] || "N/A"
                        }</td>
                        <td class="text-center">${
                          unidad.kilometrajeInicial || "N/A"
                        }</td>
                        <td class="text-center">${
                          unidad.kilometrajeActual || "N/A"
                        }</td>
                        <td class="text-center">${
                          unidad.capacidadTotal || "N/A"
                        }</td>
                        <td class="text-center">${
                          new Date(unidad.fechaDekra)
                            .toISOString()
                            .split("T")[0] || "N/A"
                        }</td>
                        <td class="text-center">${(
                          estadoMap[unidad.idEstado] || "N/A"
                        ).toUpperCase()}</td>
                    `;
