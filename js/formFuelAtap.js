(async function () {
  ...
  async function fetchUnits() {
    const API_UNIDADES = "http://localhost:18026/api/unidades/tipoUnidad/Moto";
    try {
  ...
  async function getUnitData() {
    ...
    const API_UNIDAD = `http://localhost:18026/api/unidades/${unidad}`;
    ...
    try {
      ...
      showToast(
        "Error",
        "Ocurrió un problema al obtener los datos de la unidad"
      );
    }
  }
  ...
  async function postRegistroCombustible() {
    ...
    const kilometrajeIngresado = parseInt(
    ...
    if (kilometrajeIngresado < kilometrajeActual) {
      showToast(
        "Error",
        `El kilometraje ingresado no puede ser menor que el kilometraje actual de la unidad (${kilometrajeActual}).`
      );
      return;
    }
    ...
    const data = {
      ...
      litrosAproximados: parseInt(
        document.getElementById("approxLiters").value
      ),
    ...
    try {
      ...
      showToast(
        "Error",
        "Ocurrió un problema al guardar el registro de combustible"
      );
    }
  }

  document
    .getElementById("search-fuel-button")
  ...
  async function getUltimoRegistroCombustible() {
    ...
    try {
      const response = await axios.get(
        `http://localhost:18026/api/combustibleATAP/numeroUnidad/${unidad}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (!registros || registros.length === 0) {
        showToast(
          "Error",
          `No hay registros de combustible para la unidad ${unidad}.`
        );
        return;
      }
      ...
      if (fuelLog.length > 0) {
        ...
        document.getElementById("invoiceNumber").value =
          latestLog.numeroFactura;
        document.getElementById("authorizationNumber").value =
          latestLog.numeroAutorizacion;
        document.getElementById("fuelType").value = latestLog.tipoCombustible;
        document.getElementById("approxLiters").value =
          latestLog.litrosAproximados;
        ...
        document.getElementById("date").value = new Date(latestLog.fecha)
          .toISOString()
          .split("T")[0];
        ...
        showToast(
          "Error",
          `No hay registros activos para la unidad ${unidad}.`
        );
      }
    } catch (error) {
      if (error.response && error.response.status === 403) {
        showToast(
          "Error",
          "Acceso denegado. Verifique su token de autenticación."
        );
      } else {
  ...
  async function putRegistroCombustible() {
    ...
    try {
      const response = await axios.put(
        `http://localhost:18026/api/combustibleATAP/${idCombustibleATAP}`,
        data,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      showToast(
        "Error",
        "Ocurrió un problema al actualizar el registro de combustible"
      );
    }
  }
  ...
  async function deleteRegistroCombustible() {
    ...
    try {
      ...
      showToast(
        "Error",
        "Ocurrió un problema al eliminar el registro de combustible"
      );
    }
  }
