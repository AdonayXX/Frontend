...
(async function () {
  ...
  document
    .querySelector("#choferControlKm")
    .addEventListener("change", function () {
      const idUnidad = document.querySelector("#choferControlKm").value;
      const unidadFiltrada = listaUnidadesMoto.find(
        (unidad) => unidad.id === parseInt(idUnidad)
      );

      fillChoferes(listaChoferes, unidadFiltrada.choferDesignado);
      document.querySelector("#placaUnidad").value =
        unidadFiltrada.numeroUnidad;
      ...
      kmS.addEventListener("blur", function () {
        if (
          kmS.value === "" ||
          parseInt(kmS.value) < unidadFiltrada.kilometrajeActual
        ) {
          kmS.value = unidadFiltrada.kilometrajeActual;
        }
      });
  ...
  function getDataControlKm() {
    ...
    const fechaMantenimiento = formatDate(
      document.getElementById("dateControlKm").value
    );
    ...
    if (
      ...
      isNaN(idUnidad) ||
      !fechaMantenimiento ||
      isNaN(kilometrosEntrada) ||
      isNaN(kilometrosSalida) ||
      !lugarVisitado
    ) {
  ...
  async function postDataControlKm(dataControlKm, numplaca) {
    try {
      ...
      if (response) {
        ...
        if (unidadFind) {
          await actulizarUnidad(
            unidadFind,
            dataControlKm.KilometrosEntrada,
            numplaca
          );
        }
      }
  ...
  async function getUnidadPlaca(numUnidad) {
    try {
      const API_URL = `http://localhost:18026/api/unidades/${numUnidad}`;
  ...
  async function actulizarUnidad(
    dataunidadplaca,
    kilometrajeActualizar,
    numplaca
  ) {
    try {
      ...
      const ultimoMantenimientoFecha =
        convertISOStringToDate(dataunidadplaca.ultimoMantenimientoFecha) ||
        null;
      const ultimoMantenimientoKilometraje =
        parseInt(dataunidadplaca.ultimoMantenimientoKilometraje) || null;
      ...
      const response = axios.put(
        `http://localhost:18026/api/unidades/${numplaca}`,
        dataUnidad,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  function fillUnitUser(listaChoferes, unidades, usuario) {
    const choferusuarioFiltrado = listaChoferes.find(
      (chofer) => chofer.cedula === usuario.usuario.Identificacion
    );
    // Filtrar unidades de tipo id 3 (Motos)
    const moto = unidades.filter(
      (unidad) =>
        unidad.idTipoUnidad === 3 &&
        unidad.choferDesignado === choferusuarioFiltrado.idChofer
    );
  ...
  document
    .getElementById("formKilometerControl")
