function openAccomp(cedula) {
  ...
  const API_URL = `http://localhost:18026/api/chofer/cedula/${cedula}`;
  axios
    .get(API_URL, {
      headers: {
    ...
    .then((response) => {
      ...
      if (chofer.nombreCE1 === null || chofer.nombreCE1 === undefined) {
        tableBody.innerHTML =
          '<tr><td colspan="4">No hay acompañantes registrados</td></tr>';
      } else {
        tableBody.innerHTML = `
                    <tr>
                        <td>${chofer.nombreCE1} ${chofer.apellido1CE1} ${
          chofer.apellido2CE1
        }</td>
    ...
    .catch((error) => {
      ...
      showToast(
        "Error",
        "Error al obtener los datos de contacto de emergencia."
      );
    });
}
  ...
  choferes.forEach((chofer) => {
    ...
    row.innerHTML = `
            ...
            <td class="text-center">${
              new Date(chofer.vencimientoLicencia).toISOString().split("T")[0]
            }</td>
            <td>${chofer.estadoChofer}</td>
            <td>
                <button class="btn btn-outline-primary btn-sm" onclick="openAccomp('${
                  chofer.cedula
                }')">
                    <i class="bi bi-eye"></i>
                </button>
