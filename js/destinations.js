async function loadDestinations() {
  try {
    ...
    const response = await axios.get("http://localhost:18026/api/rutas", {
      headers: {
...
async function loadEspecialidades() {
  ...
  try {
    ...
    document
      .querySelectorAll('#espe input[type="checkbox"]')
      .forEach((checkbox) => {
        checkbox.disabled = true;
      });

    document
      .querySelectorAll("#espe .btn-outline-danger")
      .forEach((deleteBtn) => {
    ...
    const handleSelectDestinosChange = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.get(
          "http://localhost:18026/api/rutaEspecialidad",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        const selectedRuta = rutas.find(
          (ruta) => ruta.IdRuta === selectedRutaId
        );
          ...
          document
            ...
            .forEach((checkbox) => {
              checkbox.addEventListener("change", function () {
                const deleteBtn = this.closest("tr").querySelector(
                  ".btn-outline-danger"
                );
                deleteBtn.disabled = true;
              });
        ...
        const especialidadesArray = selectedRuta.Especialidades.map(
        ...
        especialidadesMarcadasInicial = espeEncontrada.map(
          (especialidad) => especialidad.idEspecialidad
        );
        renderTableEspecialidades(
          especialidades,
          especialidadesMarcadasInicial
        );
        ...
        document.querySelectorAll("#espe tr").forEach((row) => {
          ...
          if (checkbox && deleteBtn) {
            deleteBtn.disabled = !especialidadesMarcadasInicial.includes(
        ...
        document
          ...
          .forEach((checkbox) => {
            checkbox.addEventListener("change", function () {
              ...
              deleteBtn.disabled =
                !especialidadesMarcadasInicial.includes(idEspecialidad) ||
                !this.checked;
            });
          });
      } catch (error) {
    ...
    document
      .querySelector("#select-destinos")
      .addEventListener("change", handleSelectDestinosChange);

    const handleBtnGuardar2Click = async () => {
      const rutaSeleccionada = document
        .getElementById("select-destinos")
        .value.trim();
      ...
      if (!rutaSeleccionada) {
        showToast(
          "Error",
          "Debes seleccionar un destino antes de asignar una especialidad."
        );
        return;
      }

      document
        .querySelectorAll('#espe input[type="checkbox"]:checked')
        .forEach((checkbox) => {
          const idEspecialidad = parseInt(checkbox.dataset.id);
      ...
      try {
        ...
        await axios.post("http://localhost:18026/api/rutaEspecialidad", data, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
    ...
    document
      .getElementById("BtnGuardar2")
      .addEventListener("click", handleBtnGuardar2Click);
...
function renderTableEspecialidades(
  especialidades,
  especialidadesMarcadasInicial = []
) {
  const tableBody = document.getElementById("espe");
  ...
  especialidades.forEach((especialidad) => {
    ...
    const checked = especialidadesMarcadasInicial.includes(
...
document.getElementById("BtnGuardarUbi").addEventListener("click", async () => {
  ...
  const apiUrl = "http://localhost:18026/api/rutas";
  const token = localStorage.getItem("token");
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button"  class="btn btn-danger" id="eliminarUbicacion" onclick='deleteDestination(${JSON.stringify(
                          idDestino
                        )})'>Eliminar</button>
                    </div>
                </div>
...
async function deleteDestination(idRuta) {
  try {
    const token = localStorage.getItem("token");
    await axios.delete(`http://localhost:18026/api/rutas/${idRuta}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
async function deleteEspecialidad(idEspecialidad, idRuta) {
  try {
    ...
    if (!idRuta) {
      showToast(
        "Error",
        "Debes seleccionar un destino antes de eliminar la especialidad."
      );
    ...
    await axios.delete(
      `http://localhost:18026/api/rutaEspecialidad/${idRuta}/${idEspecialidad}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    showToast(
      "Error",
      "No se pudo eliminar la especialidad asignada al destino seleccionado."
    );
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-danger" id="eliminarEspecialidad" onclick='deleteEspecialidad(${JSON.stringify(
                          idEspecialidad
                        )}, ${JSON.stringify(idRuta)})'>Eliminar</button>
                    </div>
                </div>
            </div>
        </div>
