    ...
    if (activeAcompanantes === 0) {
      ...
      acompanantes.forEach((accomp) => {
        const telefonoCompleto =
          accomp.Telefono2 !== 0
            ? `${accomp.Telefono1}/${accomp.Telefono2}`
            : `${accomp.Telefono1}`;
        ...
        const row = `
                    <tr>
                        ...
                        <td class="actions">
                        <button class="btn btn-outline-primary btn-sm" data-EditCompanion='${JSON.stringify(
                          accomp
                        )}' onclick = "editAccomp(this)" ><i class="bi bi-pencil-square"></i></button>
                             <button class="btn btn-outline-danger btn-sm" onclick="companionDelete(${
                               accomp.IdAcompanante
                             }, '${nombreCompleto}', '${IdentificacionComp}')"><i class="bi bi-trash"></i></button>
                        </td>
                    </tr>
                `;
  ...
  try {
    ...
    if (patient) {
      ...
      patient.forEach((patient) => {
        ...
        const telefonoCompleto =
          patient.Telefono2 !== 0
            ? `${patient.Telefono1} / ${patient.Telefono2}`
            : `${patient.Telefono1}`;
        ...
        <i class="bi bi-eye"></i>
            ...
            <td class='text-center'>
              ${
                dontShowButton
                  ? "Sin datos"
                  : `<button class="btn btn-outline-success btn-sm" onclick="getLocation('${patient.Latitud}', '${patient.Longitud}')"><i class="bi bi-geo-alt-fill"></i></button>`
              }
            </td>
            <td class='text-center'>
              <button class="btn btn-outline-primary btn-sm" data-acompanantes='${JSON.stringify(
                patient.acompanantes
              )}' onclick='openAccomp(this)'><i class="bi bi-eye"></i></button>
              <button class="btn btn-outline-success btn-sm btnAddComp" data-bs-toggle="modal" data-bs-target="#addAccomp" onclick="companionAdd(${
                patient.IdPaciente
              })"><i class="bi bi-person-plus"></i></button>
            </td>
            <td class="actions">
            <button class="btn btn-outline-primary btn-sm" data-pacientes='${JSON.stringify(
              patient
            )}' onclick = "patientEdit(this)"><i class="bi bi-pencil-square"></i></button>
              <button class="btn btn-outline-danger btn-sm" onclick="patientDelete(${
                patient.IdPaciente
              }, '${nombreCompleto}', '${
...
window.openAccomp = async function (button) {
  ...
  let modalAcomp = new bootstrap.Modal(
    document.getElementById("showAccomp"),
    {}
  );
  modalAcomp.show();
...
window.editAccomp = function (button) {
  ...
  let modalAcomp = new bootstrap.Modal(
    document.getElementById("addAccomp"),
    {}
  );
  modalAcomp.show();
  ...
  document
    .querySelector("#saveChangesCompanion")
  ...
  async function addEditedCompanion(companionData, IdAcompanante) {
    try {
      const token = localStorage.getItem("token");
      const API_URL = `http://localhost:18026/api/acompanantes/${IdAcompanante}`;
...
function addCompanion(idPacienteCapturado) {
  ...
  const acompananteApellido1 = document
    .querySelector(`#firstlastname`)
    .value.trim();
  const acompananteApellido2 = document
    .querySelector(`#secondlastname`)
    .value.trim();
  const acompananteIdentificacion = document
    .querySelector(`#identification`)
    .value.trim();
  const acompananteTelefono1 = parseInt(
    document.querySelector(`#phone1`).value.trim()
  );
  const acompananteTelefono2 = parseInt(
    document.querySelector(`#phone2`).value.trim() || "0"
  );
  const acompananteParentesco = document
    .querySelector(`#parentesco`)
    .value.trim();
...
function applyMaskBasedOnType() {
  ...
  switch (tipoIdentificacion) {
    ...
    case "Número de Asegurado":
      identificacionInput.setAttribute(
        "data-mask",
  ...
  applyIdentificationMask(
    "identificacion",
    identificacionInput.getAttribute("data-mask")
  );
}
...
async function companionDelete(IdAcompanante, nombreCompleto, Identificacion) {
  let modal = new bootstrap.Modal(
    document.getElementById("confirmDeleteModalComp"),
    {
      backdrop: "static",
      keyboard: false,
    }
