...
function getUserRegisterData() {
  const userIdentification = document.querySelector(
    "#userRegisterIdentification"
  ).value;
  const userName = document.querySelector("#userRegisterName").value;
  const userFirstLastName = document.querySelector(
    "#userRegisterFirstlastname"
  ).value;
  const userSecondLastName = document.querySelector(
    "#userRegisterSecondlastname"
  ).value;
...
async function saveUser(userData) {
  try {
    ...
    if (error.response && error.response.status === 400) {
      ...
      if (
        errorMessage.includes("Ya existe un usuario con esa identificación")
      ) {
...
async function deleteButton(userId, nombreCompleto, identificacion) {
  let modal = new bootstrap.Modal(
    document.getElementById("confirmDeleteModalUser"),
    {
      backdrop: "static",
      keyboard: false,
    }
...
async function deleteUser(userId) {
  try {
    ...
    showToast(
      "Ups!",
      "Ocurrió un problema durante la eliminación del usuario."
    );
  }
}
              ...
              <tr>
                ...
                <td class="text-center">
                 ${
                   !isActive
                     ? `
                <button class="btn btn-outline-primary btn-sm" data-user='${JSON.stringify(
                  usuario
                )}' onclick='sendUserData(this)'>
                    <i class="bi bi-pencil-square"></i>
                </button>
                <button class="btn btn-outline-danger btn-sm" onclick="deleteButton('${
                  usuario.IdUsuario
                }', '${nombreCompleto}', '${usuario.Identificacion}')">
...
window.sendUserData = function (button) {
  ...
  function fillUserFields(user) {
    document.getElementById("userIdentificationEdit").value =
      user.Identificacion;
