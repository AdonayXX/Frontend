...
async function loadViajes() {
  try {
    ...
    const response = await axios.get(
      "http://localhost:18026/api/viaje/registro",
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
function renderTable(viajes) {
  ...
  viajes.forEach((viaje) => {
    ...
    row.innerHTML = `
            ...
            <td>
                <button class="btn btn-outline-primary btn-sm" data-bs-toggle="modal" data-bs-target="#ViajeModal" onclick='getACitas(${JSON.stringify(
                  viaje
                )})'>
                    <i class="bi bi-eye"></i>
                </button>
...
function getACitas(cita) {
  try {
    ...
    const infoAdicional = [cita.idCita, cita.NombrePaciente];
