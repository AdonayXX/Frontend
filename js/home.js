...
async function mostrarProximosViajes() {
  try {
    ...
    const viajesRespuesta = await axios.get(
      "http://localhost:18026/api/viaje/destinos",
      {
        headers: {
    ...
    const viajesHoyUTC = viajes.filter((viaje) => {
      ...
      const fechaViajeUTC = new Date(
        Date.UTC(
          fechaViaje.getUTCFullYear(),
          fechaViaje.getUTCMonth(),
          fechaViaje.getUTCDate(),
          0,
          0,
          0
        )
    ...
    const proximosViajesContainer = document.getElementById(
      "proximosViajesContainer"
    );
    proximosViajesContainer.innerHTML = "";

    if (viajesHoyUTC.length === 0) {
      const mensajeNoViajes = document.createElement("p");
      mensajeNoViajes.textContent =
        "No hay viajes asignados para el día de hoy.";
      ...
      Object.keys(viajesPorDestino).forEach((destino) => {
        ...
        viajeElement.innerHTML = `
                    ...
                    <div class="card-body text-center">
                        ...
                        <h5 class="card-title">${
                          new Date(viaje.fechaInicioViaje)
                            .toISOString()
                            .split("T")[0]
                        }</h5>
                        <p class="card-text fs-5">Destino: ${letraMayuscula(
                          viaje.ubicacionDestino
                        )}</p>
                          ${
                            viajesDestino.length > 1
                              ? `<span class="badge position-absolute bottom-0 end-0" style="background-color: #094079; border-radius: 50%; margin:5px;" >${viajesDestino.length}</span>`
                              : ""
                          }
                    </div>
                `;
...
async function contarCitasHome() {
  try {
    ...
    const citasRespuesta = await axios.get("http://localhost:18026/api/cita", {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    ...
    const viajesRespuesta = await axios.get(
      "http://localhost:18026/api/viaje",
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    document.getElementById("contadorCitasFinalizadas").textContent =
      citasFinalizadas.length;
    document.getElementById("contadorCitasCanceladas").textContent =
      citasCanceladas.length;
    document.getElementById("contadorViajesFinalizados").textContent =
...
async function mostrarUnidadesPorDekra() {
  try {
    ...
    const token = localStorage.getItem("token");
    const unidadesRespuesta = await axios.get(
      "http://localhost:18026/api/unidades",
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    const unidadesPorVencer = dataUnidades.unidades.filter((unidad) => {
      ...
      const diasRestantes = Math.floor(
        (fechaVencimiento - ahora) / (1000 * 60 * 60 * 24)
      );
...
async function mostrarChoferesPorVencer() {
  try {
    ...
    const choferesRespuesta = await axios.get(
    ...
    const choferesPorVencer = dataChoferes.choferes.filter((chofer) => {
      ...
      return (
        diasRestantes <= 30 &&
        diasRestantes > 0 &&
        chofer.estadoChofer === "Activo"
      );
    });

    const choferesPorVencerContainer =
      document.getElementById("proximoschoferes");
    ...
    if (choferesPorVencer.length === 0) {
      ...
      tituloFechasChofer.classList.add(
        "text-center",
        "mb-4",
        "mt-5",
        "rounded"
      ...
      choferesPorVencer.forEach((chofer) => {
        ...
        choferElement.innerHTML = `
                    <div class="headerchofer card-header text-center text-white" style="background-color: #4267ad;">
                        <h5 class="fw-bolder">${chofer.nombre} ${
          chofer.apellido1
        }</h5>
                    </div>
                    <div class="card-body text-center">
                        ...
                        <h5 class="card-title">${
                          chofer.vencimientoLicencia.split("T")[0]
                        }</h5>
...
async function mostrarUnidadesPorKilometraje() {
  try {
    ...
    const unidadesPorKilometraje = dataUnidades.filter((unidad) => {
      if (unidad.idEstado === 1) {
        const ultimoKilometraje =
          unidad.ultimoMantenimientoKilometraje || unidad.kilometrajeInicial;
        const kilometrajeActual = unidad.kilometrajeActual;
        const frecuenciaKilometraje = unidad.valorFrecuenciaC;
        const adelantoKilometraje =
          frecuenciaKilometraje * (unidad.adelanto / 100);

        return (
          kilometrajeActual - ultimoKilometraje >=
          frecuenciaKilometraje - adelantoKilometraje
        );
      }
      return false;
    });

    const unidadesPorKilometrajeContainer = document.getElementById(
      "proximosMantenimientoKilometraje"
    );
    ...
    if (unidadesPorKilometraje.length === 0) {
      ...
      tituloMantenimientoKilometraje.textContent =
        "Mantenimiento por Kilometraje";
      tituloMantenimientoKilometraje.classList.add(
        "text-center",
        "mt-4",
        "mb-4",
        "rounded"
      );
      unidadesPorKilometrajeContainer.appendChild(
        tituloMantenimientoKilometraje
      );
      ...
      unidadesPorKilometraje.forEach((unidad) => {
        ...
        unidadElement.innerHTML = `
                    ...
                    <div class="card-body text-center">
                        <h5 class="card-title">Último Kilometraje: ${
                          unidad.ultimoMantenimientoKilometraje ||
                          unidad.kilometrajeInicial
                        }</h5>
                        <h5 class="card-title">Kilometraje Actual: ${
                          unidad.kilometrajeActual
                        }</h5>
                        <h5 class="card-title">Próximo Mantenimiento: ${
                          (unidad.ultimoMantenimientoKilometraje ||
                            unidad.kilometrajeInicial) + unidad.valorFrecuenciaC
                        }</h5>
                    </div>
                `;
