...
function mostrarCampos() {
  ...
  var contenedorSelectSalida = document.getElementById(
    "contenedorSelectSalida"
  );
  var contenedorSelectDestino = document.getElementById(
    "contenedorSelectDestino"
  );
  var contenedorEscribirSalida = document.getElementById(
    "contenedorEscribirSalida"
  );
  var contenedorEscribirDestino = document.getElementById(
    "contenedorEscribirDestino"
  );
...
function loadToastTemplate(callback) {
  fetch("toast-template.html")
    ...
    .catch((error) =>
      console.error("Error al cargar la plantilla de toast:", error)
    );
}
...
document
  .getElementById("removeCompanion")
  .addEventListener("click", function () {
    if (acompananteCount > 0) {
      const acompDiv = document.getElementById(
...
function ObtenerUnidades() {
  return axios
    .get(`${url}api/unidadProgramatica`)
    .then((response) => {
...
async function getVales() {
  try {
    ...
    if (vales.length > 0) {
      ...
      const [lastYear, lastMonth, lastNumber] = lastId.split("-");
      const currentYear = new Date().getFullYear();
      const currentMonth = String(new Date().getMonth() + 1).padStart(2, "0");

      let newNumber;
      if (currentYear === parseInt(lastYear) && currentMonth === lastMonth) {
        newNumber = String(parseInt(lastNumber) + 1).padStart(3, "0");
      } else {
        newNumber = "001";
      }
      const nuevoId = `${currentYear}-${currentMonth}-${newNumber}`;
      ...
      document.getElementById(
        "valeId"
      ).textContent = `${currentYear}-${currentMonth}-001`;
    }
  } catch (error) {
    console.error("Hubo un problema con la operación de obtención:", error);
