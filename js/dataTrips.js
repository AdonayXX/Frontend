...
(function () {
  ...
  async function obtenerCitas() {
    try {
      ...
      citasCombinadas = combinarCitasYViajes(
        citas,
        viajes,
        relacionesViajesCitas
      );
  ...
  async function cargarCitas() {
    try {
      const URL_CITAS = "http://localhost:18026/api/viajeCita";
  ...
  async function cargarViajes() {
    try {
      const URL_VIAJES = "http://localhost:18026/api/viaje";
  ...
  async function cargarRelacionesViajesCitas() {
    try {
      const URL_RELACIONES = "http://localhost:18026/api/viaje/relaciones";
      ...
      console.error(
        "Error al obtener las relaciones entre viajes y citas:",
        error
      );
      showToast(
        "Error",
        "Ocurrió un problema al obtener las relaciones entre viajes y citas"
      );
    }
  }
  ...
  function aplicarFiltros() {
    ...
    let citasFiltradas = citasCombinadas.filter((cita) => {
      ...
      const cumpleFecha =
        !fechaFiltro || formatISODate(citaDate) === fechaFiltro;
      return cumpleFecha;
    });
  ...
  function mostrarCitas(citas) {
    $(document).ready(function () {
      ...
      citas.forEach((cita) => {
        ...
        rows += `
              <tr data-paciente="${cita.idPaciente}" data-nombrepaciente="${
          cita.Paciente
        }" data-idcita="${cita.idCita}" data-ubicaciondestino="${
          cita.idUbicacionDestino
        }"  data-fechacita="${formattedFechaCita}" data-horacita="${
          cita.horaCita
        }" data-traslado="${cita.Traslado}" data-camilla="${
          cita.camilla
        }" data-lugarsalida="${cita.ubicacionOrigen}" data-estado="${
          cita.estadoCita
        }" data-idviaje="${cita.idViaje || ""}">
                <td><input type="checkbox" class="cita-checkbox" value="${
                  cita.idCita
                }" ${cita.estadoCita === "Asignada" ? "checked" : ""} ${
          cita.estadoCita === "En curso" ||
          ...
          cita.estadoCita === "Cancelada"
            ...
            : ""
                ...
                <td class="text-center">
                  <button class="btn btn-warning ausenteBtn" data-bs-toggle="modal" data-bs-target="#ausenteModal" data-idcita="${
                    cita.idCita
                  }">Ausente</button>
                </td>
              </tr>
  ...
  async function desasociarCitaDelViaje(idCita) {
    const url = `http://localhost:18026/api/viaje/cita/${idCita}`;
    ...
    try {
      ...
      console.error(
        "Error al desasociar la cita del viaje:",
        error.response.data
      );
      throw error;
    }
  ...
  function checkcamilla() {
    const checkboxes = document.querySelectorAll(
      ".cita-checkbox:checked:not(:disabled)"
    );
    let camilla = 0;
    let totalCitas = 0;
  ...
  async function cargarUnidades() {
    try {
      const URL_UNIDADES =
        "http://localhost:18026/api/ViajeUnidades/ambulancia";
  ...
  function actualizarChofer() {
    ...
    const unidadSeleccionada =
      selectUnidades.options[selectUnidades.selectedIndex];
    ...
    if (unidadSeleccionada && unidadSeleccionada.dataset.choferId) {
      ...
      choferesSelect.innerHTML =
        "<option selected>Seleccionar Chófer...</option>";
    }
  }
  ...
  function obtenerCitasSeleccionadas() {
    return Array.from(citasSeleccionadasGlobal)
      .map((idCita) => {
        const checkbox = document.querySelector(
          `.cita-checkbox[value="${idCita}"]`
        );
        if (checkbox) {
          const row = checkbox.closest("tr");
          return {
  ...
  async function crearViajes() {
    ...
    const unidadSeleccionadaTexto =
      idUnidadElement.options[idUnidadElement.selectedIndex].textContent;
    document.getElementById("unidadSeleccionadaModal").textContent =
      unidadSeleccionadaTexto;

    const citasSeleccionadasList = document.getElementById(
      "citasSeleccionadasList"
    );
  ...
  document
    .getElementById("confirmarViajesBtn")
    .addEventListener("click", crearViajes);
  ...
  async function enviarViajesConfirmados() {
    ...
    const getIdViaje = `http://localhost:18026/api/viaje/unidades/${idUnidad}/${fechaInicio}`;
    const idViaje = await returnIdViaje(getIdViaje);

    if (idViaje === "Error") {
      try {
        await axios.post("http://localhost:18026/api/viaje", nuevoViaje, {
          headers: { Authorization: `Bearer ${token}` },
        });
      ...
      try {
        await axios.put(
          `http://localhost:18026/api/viaje/actualizar/viajeCita`,
          asignarCita,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
  ...
  document
    .getElementById("confirmarViajesBtnModal")
    .addEventListener("click", enviarViajesConfirmados);
  ...
  async function marcarCitaComoAusente() {
    ...
    const url = `http://localhost:18026/api/cita/${idCita}`;
    const urlDesasociarCita = `http://localhost:18026/api/viaje/cita/${idCita}`;
    ...
    try {
      ...
      console.error(
        "Error al marcar la cita como ausente:",
        error.response.data
      );
    }
  }
  ...
  document
    .getElementById("guardarAusenciaBtn")
    .addEventListener("click", marcarCitaComoAusente);
  ...
  function handleSearchTrips() {
    ...
    let rows = Array.from(
      document.getElementById("tableTrips").getElementsByTagName("tr")
    );
