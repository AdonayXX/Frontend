async function generalesPdf() {
  try {
    ...
    const existingPdfBytes = await axios.get(pdfUrl, {
    ...
    const response1 = await axios.get("http://localhost:18026/api/vales", {
      headers,
    });
    const response2 = await axios.get(
      "http://localhost:18026/api/registrocombustible",
      { headers }
    );
    const response3 = await axios.get("http://localhost:18026/api/cita", {
      headers,
    });
    ...
    const valesFiltrados = datosVales.filter((vale) => {
      ...
      return (
        fechaSolicitud >= fromDate &&
        fechaSolicitud <= toDate &&
        vale.NombreEstado === "Aprobado"
      );
    });

    const cantidadFuncionariosTrasladados = valesFiltrados.reduce(
      (sum, vale) => {
        let count = 0;
    ...
    const gasolinaLitros = datosRegistros
      .filter((registro) => {
        ...
        return (
          fecha >= fromDate &&
          fecha <= toDate &&
          registro.tipoCombustible === "Gasolina"
        );
      })
      .reduce(
        (sum, registro) => sum + parseFloat(registro.litrosAproximados),
        0
      );
    ...
    const dieselLitros = datosRegistros
      .filter((registro) => {
        ...
        return (
          fecha >= fromDate &&
          fecha <= toDate &&
          registro.tipoCombustible === "Diesel"
        );
      })
      .reduce(
        (sum, registro) => sum + parseFloat(registro.litrosAproximados),
    ...
    const dieselKilometraje = datosRegistros
      ...
      .reduce(
        (sum, registro) => sum + parseFloat(registro.kilometrajeRecorrido),
        0
      );
    ...
    const pacientes = datosCitas.filter((paciente) => {
      ...
      return (
        fechaCita >= fromDate &&
        fechaCita <= toDate &&
        paciente.estadoCita === "Finalizada"
      );
    });
    ...
    const monthName = fromDateValue.toLocaleString("default", {
      month: "long",
    });
    ...
    const gasolinaLitrosString = gasolinaLitros
      .toFixed(2)
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const gasolinaLitrosWidth = helveticaFont.widthOfTextAtSize(
      gasolinaLitrosString,
      8
    );
    ...
    const dieselLitrosString = dieselLitros
      .toFixed(2)
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const dieselLitrosWidth = helveticaFont.widthOfTextAtSize(
      dieselLitrosString,
      8
    );
    ...
    const gasolinaKilometrajeString = gasolinaKilometraje
      .toFixed(2)
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const gasolinaKilometrajeWidth = helveticaFont.widthOfTextAtSize(
      gasolinaKilometrajeString,
      8
    );
    const gasolinaKilometrajeX = 273 + (50 - gasolinaKilometrajeWidth) / 2;
    const gasolinaKilometrajeFormatted = gasolinaKilometrajeString.replace(
      ".00",
      ""
    );
    ...
    const dieselKilometrajeString = dieselKilometraje
      .toFixed(2)
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const dieselKilometrajeWidth = helveticaFont.widthOfTextAtSize(
      dieselKilometrajeString,
      8
    );
    const dieselKilometrajeX = 273 + (50 - dieselKilometrajeWidth) / 2;
    const dieselKilometrajeFormatted = dieselKilometrajeString.replace(
      ".00",
      ""
    );
    ...
    const valesWidth = helveticaFont.widthOfTextAtSize(
      valesLength.toString(),
      8
    );
    ...
    const totalWidth = helveticaFont.widthOfTextAtSize(
    ...
    const pacientesWidth = helveticaFont.widthOfTextAtSize(
      pacientesLength.toString(),
      8
    );
    ...
    const funcionariosWidth = helveticaFont.widthOfTextAtSize(
      funcionariosLength,
      8
    );
    ...
    const gasolinaLitrosStringG = gasolinaLitros
      .toFixed(2)
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const gasolinaLitrosWidthG = helveticaFont.widthOfTextAtSize(
      gasolinaLitrosStringG,
      8
    );
    ...
    const dieselLitrosStringG = dieselLitros
      .toFixed(2)
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const dieselLitrosWidthG = helveticaFont.widthOfTextAtSize(
      dieselLitrosStringG,
      8
    );
    ...
    const gasolinaKilometrajeStringG = gasolinaKilometraje
      .toFixed(2)
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const gasolinaKilometrajeWidthG = helveticaFont.widthOfTextAtSize(
      gasolinaKilometrajeStringG,
      8
    );
    const gasolinaKilometrajeXG = 530 + (50 - gasolinaKilometrajeWidthG) / 2;
    const gasolinaKilometrajeFormattedG = gasolinaKilometrajeStringG.replace(
      ".00",
      ""
    );
    ...
    const dieselKilometrajeStringG = dieselKilometraje
      .toFixed(2)
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const dieselKilometrajeWidthG = helveticaFont.widthOfTextAtSize(
      dieselKilometrajeStringG,
      8
    );
    const dieselKilometrajeXG = 530 + (50 - dieselKilometrajeWidthG) / 2;
    const dieselKilometrajeFormattedG = dieselKilometrajeStringG.replace(
      ".00",
      ""
    );
    ...
    const valesWidthG = helveticaFont.widthOfTextAtSize(
      valesLengthG.toString(),
      8
    );
    ...
    const totalWidthG = helveticaFont.widthOfTextAtSize(
      totalLengthG.toString(),
      8
    );
    ...
    const pacientesWidthG = helveticaFont.widthOfTextAtSize(
      pacientesLengthG.toString(),
      8
    );
    ...
    const funcionariosWidthG = helveticaFont.widthOfTextAtSize(
      funcionariosLengthG,
      8
    );
...
async function generalesExcel() {
  try {
    ...
    worksheet.getCell(cellTotalFuncionariosPacientes).value =
      cantidadFuncionariosTrasladados + pacientes.length;
    worksheet.getCell(cellPacientes).value = pacientes.length;
    worksheet.getCell(cellTotalFuncionarios).value =
      cantidadFuncionariosTrasladados;
    ...
    worksheet.getCell(cellTotalFuncionariosPacientes2).value =
      cantidadFuncionariosTrasladados + pacientes.length;
    worksheet.getCell(cellPacientes2).value = pacientes.length;
    worksheet.getCell(cellTotalFuncionarios2).value =
      cantidadFuncionariosTrasladados;
    ...
    const cellsToStyle = [
      ...
      cellGasolinaLitros,
      cellDieselLitros,
      cellGasolinaKilometraje,
      cellDieselKilometraje,
      cellValesFiltrados,
      cellTotalFuncionariosPacientes,
      cellAno,
      cellMes,
      cellNumeroMes,
      cellTotalFuncionarios,
    ];
    ...
    alert(
      "Hubo un error al modificar el archivo Excel. Por favor, intente de nuevo."
    );
  }
}
...
//             let apiUrl = `http://localhost:18026/api/reporteMantenimiento/porFecha?fechaInicio=${fromDate.toISOString().split('T')[0]}&fechaFin=${toDate.toISOString().split('T')[0]}`;

//             if (idActividad) {
//                 apiUrl = `http://localhost:18026/api/reporteMantenimiento/porActividad?IdActividad=${idActividad}&fechaInicio=${fromDate.toISOString().split('T')[0]}&fechaFin=${toDate.toISOString().split('T')[0]}`;
//             }

//             if (unidad) {
//                 apiUrl = `http://localhost:18026/api/reporteMantenimiento/porUnidad?id=${unidad}&fechaInicio=${fromDate.toISOString().split('T')[0]}&fechaFin=${toDate.toISOString().split('T')[0]}`;
//             }
...
//         const response1 = await axios.get('http://localhost:18026/api/vales', { headers });
//         const response2 = await axios.get('http://localhost:18026/api/registrocombustible', { headers });
//         const response3 = await axios.get('http://localhost:18026/api/cita', { headers });
//         const datosVales = response1.data.vales;
//         const datosRegistros = response2.data.registros;
//         const datosCitas = response3.data;
...
async function mantenimientoExcel() {
  try {
    ...
    worksheet.getCell(cellfechaCreacion).value =
      "Creado el " + fechaCreacionString + " a las " + horaCreacionString;
    worksheet.getCell(cellrangoFecha).value =
      "De: " +
      adjustedFromDateE.toLocaleDateString() +
      " Hasta: " +
      adjustedToDate.toLocaleDateString();

    const cellsToStyle = [cellrangoFecha];
...
const btnValeExportarExcel = document.getElementById("valeExportar");

// btnValeExportar.addEventListener("click", function () {
...
// btnValeExportarExcel.addEventListener("click", function () {
//   // Obtener el valor de la fecha de inicio
//   const fechaInicio = document.getElementById("inicioVale").value;

//   // Obtener el valor de la fecha de fin
//   const fechaFin = document.getElementById("finVale").value;

//   exportarValeExcelToDate(fechaInicio, fechaFin);
// });

//REPORTE DE VALES EXCEL
async function exportarValeExcelToDate(fechaInicio, fechaFin) {
  try {
    if (!fechaInicio || !fechaFin) {
      showToast(
        "Error",
        "Ambos campos de fecha no deben estar vacíos. Ingrese la fecha de inicio y fin."
      );
      return;
    }
    ...
    try {
      const response = await axios.get(
        `http://localhost:18026/api/vales/exportar/vale/${fechaInicio}/${fechaFin}`,
        { headers }
      );
      datosVales = response.data.vales;
    } catch (apiError) {
      showToast(
        "Error",
        "No se encontraron vales en el rango de fechas especificado."
      );
      return;
    }
    ...
    const plantillaSheet = workbook.getWorksheet("Vale");
    if (!plantillaSheet) {
      throw new Error(
        "La hoja de plantilla 'VALE' no se encontró en el archivo Excel."
      );
    }

    // Crear hojas para cada vale sin duplicación
    datosVales.forEach((valeData) => {
      let sheetName = `${valeData.IdVale}`;
      let uniqueSuffix = 1;

      // Si el nombre ya existe, añade un sufijo para hacerlo único
      while (workbook.getWorksheet(sheetName)) {
        sheetName = `${valeData.IdVale}_${uniqueSuffix}`;
        uniqueSuffix++;
      }

      // Duplicar la hoja de plantilla 'VALE'
      const newSheet = workbook.addWorksheet(sheetName);

      // Recorremos todas las filas de la plantilla
      plantillaSheet.eachRow({ includeEmpty: true }, (row, rowIndex) => {
        ...
        row.eachCell({ includeEmpty: true }, (cell, colIndex) => {
          ...
          // Copiar valores y estilos de la celda
      ...
      // Rellenar datos específicos de cada vale
      ...
      newSheet.getCell("B8:C8").value = new Date().toISOString().split("T")[0];
      newSheet.getCell("K8").value = valeData.IdUnidadProgramatica || "";
      newSheet.getCell("F7:G7:H7:I7:J7").value = valeData.NombreUnidad || "";
      newSheet.getCell("I12:J12:K12").value = valeData.Acompanante1 || "";
      newSheet.getCell("I13:J13:K13").value = valeData.Acompanante2 || "";
      newSheet.getCell("I14:J14:K14").value = valeData.Acompanante3 || "";
      newSheet.getCell("I15:J15:K15").value = valeData.Acompanante4 || "";
      newSheet.getCell("I16:J16:K16").value = valeData.Acompanante5 || "";
      newSheet.getCell("B13:C13:D13:E13:F13:G13:H13").value =
        valeData.DescripcionMotivo;
      newSheet.getCell("G17:H17:I17:J17:K17").value =
        valeData.NombreSolicitante || "";
      newSheet.getCell("H18:I18:J18:K18").value = valeData.Detalle || "";
      newSheet.getCell("G25").value = horaSalida;
      newSheet.getCell("C25:D25:E25").value = fechaSolicitud;
      newSheet.getCell("C9").value = destino || "";
      newSheet.getCell("C20").value = horaSalida;
      newSheet.getCell("C19").value = fechaSolicitud;
      newSheet.getCell("K25").value = horaEntrada || "";
      newSheet.getCell("C30").value = valeData.kilometrajeInicioVale || "";
      newSheet.getCell("G30").value = valeData.kilometrajeFinalVale || "";
      newSheet.getCell("E20").value = horaEntrada || "";
      newSheet.getCell("E19").value = fechaSolicitud;
      newSheet.getCell("I25").value = fechaSolicitud;
      newSheet.getCell("C30").value = valeData.kilometrajeInicioVale || "";
      newSheet.getCell("G26").value = valeData.EncargadoCordinador || "";
      newSheet.getCell("G28").value = valeData.NombreCompletoChofer || "";
      newSheet.getCell("K5").value = valeData.numeroUnidad || "";
      newSheet.getCell("C30").value = `${
        valeData.KilometrajeInicialViaje || ""
      } km`;
      newSheet.getCell("G30").value = `${
        valeData.KilometrajeFinalViaje || ""
      } km`;
      newSheet.getCell("K25").value = valeData.HoraFinViaje || "";
      newSheet.getCell("E20").value = valeData.HoraFinViaje || "";
    });
    ...
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = `Reporte_Vale_${fechaInicio}_${fechaFin}.xlsx`;
    link.click();
  } catch (error) {
...
async function exportarValePdf() {
  try {
    ...
    try {
      ...
      showToast(
        "Error",
        "No se encontró el ID del vale. Por favor, verifique el número ingresado."
      );
      return;
    }
    ...
    const valeData = datosVale.vale;
    const fechaSolicitud = new Date(valeData.Fecha_Solicitud)
      .toISOString()
      .split("T")[0];
    ...
    worksheet.drawText(String(destino || ""), {
      x: 260,
      y: height - 205,
      size: 10,
      font: helveticaFont,
    ...
    worksheet.drawText(String(valeData.KilometrajeInicialViaje || ""), {
      x: 90,
      y: height - 480,
      size: 10,
      font: helveticaFont,
    ...
    worksheet.drawText(String(valeData.KilometrajeFinalViaje || ""), {
      x: 250,
      y: height - 480,
      size: 10,
      font: helveticaFont,
...
async function viajesPdf() {
  try {
    ...
    const response = await axios.post(
      "http://localhost:18026/api/reporteViaje",
      {
        fechaInicio,
        fechaFin,
      },
    ...
    if (
      ...
      response.data.resultados.success
    ) {
      ...
      reader.onload = function (event) {
        ...
        doc.autoTable({
          ...
          head: [encabezados],
          body: data.map((row) =>
            encabezados.map((header) => row[header] || "")
          ),
      ...
      console.error(
        "No se encontraron datos para el reporte o la solicitud no fue exitosa."
      );
    }
  } catch (error) {
...
async function viajesExcel() {
  try {
    ...
    if (
      ...
      worksheet.getCell(
        "B5"
      ).value = `Lista de Viajes del ${fechaInicio} al ${fechaFin}`;
...
//         const url = 'http://localhost:18026/api/unidades';
//         const token = localStorage.getItem('token');
...
//         const url = 'http://localhost:18026/api/actividad';
//         const token = localStorage.getItem('token');
